        ...
        <div class="parent">
          ...
          <div class="child2">
            ...
            <el-form>
              ...
              <el-form-item><el-button type="danger" round @click="buy">立即购买</el-button><el-button type="danger" @click="goToShopCar" round>加入购物车</el-button></el-form-item>
...
const userId = useUserStore().getRegularUserInfo().regularUserId//用户id
...
const goToShopCar = () =>{
  pageData.shopCar.shoppingCartId=0
  pageData.shopCar.commodityId=pageData.commodity.id
  pageData.shopCar.commodityNumber=buyNum.value
  pageData.shopCar.regularUserId=userId
  axios.post("http://localhost:9090/shopping-cart-entity/addCommodity",pageData.shopCar).then(res=>{
...
onMounted(()=>{
  ...
  getShopComments()
  getUserInfo()
  useRegularStore().clearOrderInfo()
})
...
const buy = () =>{
  if(storage.value<=0)
  {
    ...
    let index = 0;
    pageData.userInfoList.forEach((item)=>{
      if(item.detailAddress == orderInfo.address){
        index = pageData.userInfoList.indexOf(item)
      }
    })
    orderInfo.num=buyNum.value
    orderInfo.name=pageData.userInfoList[index].linkman
    orderInfo.phone=pageData.userInfoList[index].linkmanPhone
