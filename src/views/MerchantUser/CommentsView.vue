<template>
  <div>
    ...
    <div>
    <el-table stripe height="400px" :data="filterData" style="width: 100% "
      ...
      <el-form :model="commentInfo.changeInfo" label-width="80px">
        ...
        <el-card style="margin:1%">
          <el-form-item label="图片" style="width: 40%">
            <el-image v-if="commentInfo.changeInfo.commentsPhoto!=''"
                      :src="commentInfo.changeInfo.commentsPhoto"></el-image>
...
const makeSureReply = () => {
  ...
  axios.post("http://localhost:9090/comments-entity/updateCommentsById", commentInfo.changeInfo).then(res => {
    ...
    if (res.data.code === 200) {
      ...
      axios.get("http://localhost:9090/comments-order-dform-entity/getCommentsByShopId/" + myPageInfo.shopId).then(res => {
        commentsData.push(...res.data.data)
        commentsData.forEach(item => {
          let time = new Date(item.time)
          item.time.substring(10,11)
          return item.time = time.getFullYear() + '-' + (time.getMonth() + 1) + '-' + time.getDate() + ' ' + time.getHours() + ':' + time.getMinutes()
        })
      })
    } else {
      console.log("回复失败")
...
const myPageInfo = reactive({
  shopId:props.nowShop,  //固定死的商店id 需要传入商店id
  // shopId: 5,
  reply: '',
})
...
onMounted(() => {
  axios.get("http://localhost:9090/comments-order-form-entity/getCommentsByShopId/" + myPageInfo.shopId).then(res => {
    ...
    commentsData.forEach(item => {
      let time = new Date(item.time)
      item.time.substring(10,11)
      return item.time = time.getFullYear() + '-' + (time.getMonth() + 1) + '-' + time.getDate() + ' ' + time.getHours() + ':' + time.getMinutes()+':'+time.getSeconds()
    })
  })
})
</script>
